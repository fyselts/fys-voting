...
export function CsvUploadForm() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      const users = rows
        ...
        .filter(
          (u): u is { full_name: string; email: string } =>
            u !== null && !!u.email && !!u.full_name
        );
  ...
  return (
    <div className="flex w-full max-w-md flex-col gap-4 rounded-lg bg-white p-6 shadow">
      <h3 className="mb-2 text-xl font-bold">{t('upload_csv')}</h3>
      <p className="mb-2 text-sm text-gray-500">{t('upload_csv_subtitle')}</p>
      <input
        type="file"
        accept=".csv,.tsv,.txt"
        onChange={handleFileUpload}
        disabled={isPending}
        className="block w-full text-sm text-gray-500 file:mr-4 file:rounded-full file:border-0 file:bg-blue-50 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-blue-700 hover:file:bg-blue-100 dark:file:bg-zinc-700 dark:file:text-gray-200"
      />
      {isPending && <p className="text-blue-500">{t('processing')}</p>}
      {message && (
        <p
          className={`mt-2 text-sm font-semibold ${message.type === 'success' ? 'text-green-600' : 'text-red-600'}`}
        >
          {message.text}
        </p>
      )}
