...
export function OtpForm({ email }: { email: string }) {
  ...
  return (
    <Card className="flex w-full max-w-md flex-col items-center gap-6">
      ...
      <InfoBox>{t('verification_code_info', { email })}</InfoBox>
      <Form action={formAction}>
        ...
        <div className="flex flex-col gap-2">
          ...
          <InputWithIcon
            ...
            pattern="\d{8}"
            maxLength={8}
            icon={<KeypadIcon className="h-5 w-5" />}
        ...
        {state?.message && (
          <p
            className={`mt-4 text-lg font-semibold ${state.success ? 'text-green-600' : 'text-red-600'}`}
          >
            {state.message}
          </p>
        )}
